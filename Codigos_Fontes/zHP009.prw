...
@version 1.0
/*/
user function zHP009()
    local aArea     := FwGetArea()
    local oBrowse
    local cIdProt   := RetCodUsr()
    local cIDHelpDsk:= POSICIONE("ZZB",2, FWXFILIAL("ZZB")+cIdProt, "ZZB_ID") 
    local lTecnico  := (POSICIONE("ZZB",1, FWXFILIAL("ZZB")+cIDHelpDsk, "ZZB_TIPO") <> '1')
    local cCondicao := ""
    local lTemUsr   := .F.
    private aRotina := {}
...
Menu de opcoes na funcao zHP009
@author Luis Felipe Oliveira
@since 14/05/2026
@version 1.0
@type function
...
static function MenuDef()
    ...
    ADD OPTION aRotina TITLE "Visualizar"    ACTION "VIEWDEF.zHP009" OPERATION 1 ACCESS 0
    ADD OPTION aRotina TITLE "Incluir"       ACTION "VIEWDEF.zHP009" OPERATION 3 ACCESS 0
    ADD OPTION aRotina TITLE "Alterar"       ACTION "VIEWDEF.zHP009" OPERATION 4 ACCESS 0
    ADD OPTION aRotina TITLE "Excluir"       ACTION "VIEWDEF.zHP009" OPERATION 5 ACCESS 0
...
Modelo de dados na funcao zHP009
@author Luis Felipe Oliveira
@since 14/05/2026
@version 1.0
@type function
...
static function ModelDef()
    ...
    local cIdProt    := RetCodUsr()
    local cIDHelpDsk := POSICIONE("ZZB",2, FWXFILIAL("ZZB")+cIdProt, "ZZB_ID") 
    local lTecnico   := iif( POSICIONE("ZZB",1, FWXFILIAL("ZZB")+cIDHelpDsk, "ZZB_TIPO") <> '1', .T., .F.)
    ...
    //Cabecalho ZZH
    oStruPai:SetProperty('ZZH_ID',     MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZH01()'              ) )  //Numero do Chamado

    oStruPai:SetProperty('ZZH_IDSOL',  MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZH02()'              ) )  //ID do Usuário HelpDesk
    oStruPai:SetProperty('ZZH_NOMSOL', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZH03()'              ) )  //Nome do Solicitante
    oStruPai:SetProperty('ZZH_IDDEP',  MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZH04()'              ) )  //ID Departamento
    oStruPai:SetProperty('ZZH_DESDEP', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZH05()'              ) )  //Descricao Departamento

    oStruPai:SetProperty('ZZH_DESTIP', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZH06()'              ) )  //Descricao Tipo Chammado
    oStruPai:SetProperty('ZZH_DESCAT', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZH07()'              ) )  //Descricao Categoria
    oStruPai:SetProperty('ZZH_DESSUC', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZH08()'              ) )  //Descricao Subcategoria
    oStruPai:SetProperty('ZZH_DTABER', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZH09()'              ) )  //Data da abertura
    oStruPai:SetProperty('ZZH_STATUS', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_StatsZZH("Codigo",    1)') )  //Status Chamado
    oStruPai:SetProperty('ZZH_DESSTS', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_StatsZZH("Descricao", 0)') )  //Desc do Status do Chamado

    //Item ZZI
    oStruFilho:SetProperty('ZZI_IDUSER', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZI01()' ) )  //ID do Usuario HelpDesk
    oStruFilho:SetProperty('ZZI_NOMUSR', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZI02()' ) )  //Nome do Usuario  
    oStruFilho:SetProperty('ZZI_DATA',   MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'DATE()'       ) )  //Data
    oStruFilho:SetProperty('ZZI_TIPINT', MODEL_FIELD_INIT, FwBuildFeature(STRUCT_FEATURE_INIPAD, 'u_InPZZI03()' ) )  //Tipo de Interacao  
    ...
    // 3-Campo obrigatorio
    //-----------------------------------------
    oStruPai:SetProperty('ZZH_IDSUBC',  MODEL_FIELD_OBRIGAT, .T. ) //ID da Subcategoria
    oStruFilho:SetProperty('ZZI_DESC',  MODEL_FIELD_OBRIGAT, .T. ) //Descricao do problema/Mensagem


    //-------------------------------------------------
    // 4-Modo de Edicao
    //-------------------------------------------------
    // Os valores comuns para o modo de edição incluem:
    // ".T.": Campo editável.
    // ".F.": Campo bloqueado (apenas leitura). 
    // "INCLUI": Editável apenas na inclusão. 
    // "ALTERA": Editável apenas na alteração.

    //Libera para edicao do campo somente se o campo anterior estiver preenchido.
    oStruPai:SetProperty('ZZH_IDTIPO', MODEL_FIELD_WHEN, { || !empty(fwfldget('ZZH_ASSUNT') ) } ) //ID Categoria
    oStruPai:SetProperty('ZZH_IDCAT',  MODEL_FIELD_WHEN, { || !empty(fwfldget('ZZH_IDTIPO') ) } ) //ID Categoria
    oStruPai:SetProperty('ZZH_IDSUBC', MODEL_FIELD_WHEN, { || !empty(fwfldget('ZZH_IDCAT' ) ) } ) //ID Subcategoria
    ...
    //-----------------------------------------
    // 5-Configuracao de gatilho
    //-----------------------------------------

    //Adicionando um gatilho do ZZH_IDSOL (ID Solicitante) para preencher o campo ZZH_NOMSOL (Nome do Solicitante).
    aAdd(aGatilhos, ;
        FwStruTrigger(;
            "ZZH_IDSOL",;           //Campo Origem
            "ZZH_NOMSOL",;          //Campo Destino
            "u_GatZZH01()",;        //Regra de Preenchimento
            .F.,;                   //Irá Posicionar
    ...
    aAdd(aGatilhos, ;
        FwStruTrigger(;
            "ZZH_IDSOL",;           //Campo Origem
            "ZZH_IDDEP",;          //Campo Destino
            "u_GatZZH02()",;        //Regra de Preenchimento
            .F.,;                   //Irá Posicionar
            "",;                    //Alias do Posicionamento
            0,;                     //Indice do Posicionamento
            '',;                    //Chave de Posicionamento
    ...
    //Adicionando um gatilho do ZZH_IDTIPO (ID Tipo) para preencher o campo ZZH_DESTIP (Descricao Tipo)
    aAdd(aGatilhos, ;
        FwStruTrigger(;
            "ZZH_IDTIPO",;          //Campo Origem
            "ZZH_DESTIP"  ,;        //Campo Destino
            "u_GatZZH03()",;        //Regra de Preenchimento
            .F.,;                   //Irá Posicionar
            "",;                    //Alias do Posicionamento
            0,;                     //Indice do Posicionamento
            '',;                    //Chave de Posicionamento
    ...
    //Adicionando um gatilho do ZZH_IDCAT (ID Categoria) para preencher o campo ZZH_DESCAT (Descricao Categoria)
    aAdd(aGatilhos, ;
        FwStruTrigger(;
            "ZZH_IDCAT",;           //Campo Origem
            "ZZH_DESCAT"  ,;        //Campo Destino
            "u_TrigZZH('ZZH_IDCAT')",;   //Regra de Preenchimento
            .F.,;                   //Irá Posicionar
            "",;                    //Alias do Posicionamento
            0,;                     //Indice do Posicionamento
            '',;                    //Chave de Posicionamento
    ...
    //Adicionando um gatilho do ZZH_IDSUBC (ID Subcategoria) para preencher o campo ZZH_DESSUC (Descricao Subcategoria)
    aAdd(aGatilhos, ;
        FwStruTrigger(;
            "ZZH_IDSUBC",;          //Campo Origem
            "ZZH_DESSUC"  ,;        //Campo Destino
            "u_TrigZZH('ZZH_IDSUBC')",;    //Regra de Preenchimento
            .F.,;                   //Irá Posicionar
            "",;                    //Alias do Posicionamento
            0,;                     //Indice do Posicionamento
            '',;                    //Chave de Posicionamento
    ...
    //Adicionando um gatilho do ZZH_STATUS (Status) para preencher o campo ZZH_DESSTS (Descricao do Status)
    aAdd(aGatilhos, ;
        FwStruTrigger(;
            "ZZH_STATUS",;          //Campo Origem
            "ZZH_DESSTS",;          //Campo Destino
            "u_TrigZZH('ZZH_STATUS')",;    //Regra de Preenchimento
            .F.,;                   //Irá Posicionar
            "",;                    //Alias do Posicionamento
            0,;                     //Indice do Posicionamento
            '',;                    //Chave de Posicionamento
    ...
    oModel := MPFormModel():New('zHP009M', bPre, bPos, bCancel, bCommit)
...
Visualizacao de dados na funcao zHP009
@author Luis Felipe Oliveira
@since 14/05/2026
@version 1.0
@type function
...
static function ViewDef()
    local oModel     := FWLoadModel('zHP009')
    local oStruPai   := FWFormStruct(2,cTabPai)
    local oStruFilho := FWFormStruct(2,cTabFilho)
    local oView
    local lTecnico   := ( POSICIONE("ZZB",2, FWXFILIAL("ZZB")+RETCODUSR(), "ZZB_TIPO") $ '23' )
    ...
    if lTecnico
        ...
        oView:addUserButton("Assumir Chamado"    , "MAGIC_BMP" , {|| zAssumiChd()                       }, /*cToolTip*/, /*nShortCut*/, /*aOptions*/, .T.)
        oView:addUserButton("Alterar Chamado"    , "MAGIC_BMP" , {|| Alert("Em construção")             }, /*cToolTip*/, /*nShortCut*/, /*aOptions*/, .F.)
    endif
    ...
    if ! oModelPad == Nil
        ...
        */
        ...
        if nOperation = MODEL_OPERATION_INSERT
            ...
            cIdTip     := oModelZZH:GetValue("ZZH_IDTIPO") // Tipo de Chamado
		    cIdCat     := oModelZZH:GetValue("ZZH_IDCAT" ) // Categoria 
		    cIdSub     := oModelZZH:GetValue("ZZH_IDSUBC") // Subcategoria
		    cIdHora    := Posicione("ZZG",1,FWxFilial("ZZG")+ cIdTip + cIdCat + cIdSub, "ZZG_IDHORA") // ID Hora
		    nHorasSLA  := Val(Posicione("ZZF",1,FWxFilial("ZZF")+ cIdHora, "ZZF_HORA")) // Quantidade de horas do SLA
...
static function zAssumiChd() 
    ...
    local cIDHelpDsk := POSICIONE("ZZB",2, FWXFILIAL("ZZB")+cIDProt, "ZZB_ID") //ID HelpDesk
    local lTecnico   := ( POSICIONE("ZZB",2, FWXFILIAL("ZZB")+RETCODUSR(), "ZZB_TIPO") $ '23') 
    local oModel     := FWModelActive()
    local oView      := FwViewActive()
    local nOperacao  := 0

    //Checa se o ID possui permissao para atender
    if lTecnico
        ...
        if nOperacao ==  MODEL_OPERATION_INSERT .or. nOperacao = MODEL_OPERATION_UPDATE
            ...
            oModel:SetValue("ZZHMASTER","ZZH_NOMTEC", USRFULLNAME(cIDProt)    ) //Nome do Atendente
...
/*/{Protheus.doc} TrigZZH
    Funcao responsavel pelos gatilhos para preencher os campos virtuais e pela limpeza 
    das informacoes em memoria quando alterado os campos de ID Tipo e ID Categoria.
    @type  User Function
    @author Luis Felipe Oliveira
    @since 27/05/2026
    @version 1.0
    @param cxCampo, Caractere, Nome do campo que irah iniciar o gatilho.
    @return cRet, Caractere, Retorna a descricao do ID conforme o campo passado na variavel cxCampo. 
    @see https://www.youtube.com/watch?v=xgWmg9DVXo4
/*/
user function TrigZZH(cxCampo)
    local aArea     := FwGetArea()
    local cIdCat    := ""
    local cDesCat   := ""
    local cIdSubc   := ""
    ...
    if cxCampo == "ZZH_IDCAT" //Categoria
        //Pega o ID da Categoria do Chamado e posiciona na tabela ZZD para retornar a descricao.
        cIdCat  := oModel:GetModel("ZZHMASTER"):GetValue("ZZH_IDCAT")
        cDesCat := Posicione("ZZD",2, FWXFILIAL("ZZD")+cIdCat, "ZZD_DESCR")
        
        //Realiza a limpeza dos campos.
        oModel:GetModel("ZZHMASTER"):LoadValue("ZZH_IDSUBC", "")
        oModel:GetModel("ZZHMASTER"):LoadValue("ZZH_DESSUC", "")

        //Retorna a descricao da Categoria.
        cRet := cDesCat

    elseif cxCampo == "ZZH_IDSUBC" //Subcategoria
            //Pega o ID da Subcategoria e posiciona na tabela ZZC para retornar a descricao.
            cIdSubc  := oModel:GetModel("ZZHMASTER"):GetValue("ZZH_IDSUBC")
            cDesSubc := Posicione("ZZE",2, FWXFILIAL("ZZE")+cIdSubc, "ZZE_DESCR")

            //Retorna a descricao da Subcategoria.
            cRet := cDesSubc
    else
        if cxCampo == "ZZH_STATUS" //Status do Chamado
            //Pega o conteudo do campo ZZH_STATUS.
            cStatus := oModel:GetModel("ZZHMASTER"):GetValue("ZZH_STATUS")

            //Atribui a descricao do Status
            do case
                case cStatus == 'A'
                    cDesStatus := "Aberto sem atendimento"
                case cStatus == 'B'
                    cDesStatus := "Em atendimento"
                case cStatus == 'C'
                    cDesStatus := "Pausado"
                case cStatus == 'D'
                    cDesStatus := "Finalizado"
                otherwise
                    cDesStatus := "Cancelado"
            endcase

            //Retona a descricao do Status.
            cRet := cDesStatus
        endif
    endif
...
/*/{Protheus.doc} InPZZH01()
Retorna o proximo numero para o preenchimento automatico da numeracao do chamado.
Preenche o campo ZZH_ID (Numeracao do Chamado).
@type user function
@author user
@since 31/05/2026
@return cRet, Caractere, ID do Solicitante
/*/
user function InPZZH01() 
	local aArea := FwGetArea()
	local cRet 	:= ""
	
	cRet := GetSXENum("ZZH","ZZH_ID")
	
	FwRestArea(aArea)
return cRet


/*/{Protheus.doc} InPZZH02()
Retorna o ID do Solicitante cadastrado no HelpDesk.
Preenche o campo ZZH_IDSOL (ID Solicitante).
@type user function
@author user
@since 31/05/2026
@return cRet, Caractere, ID do Solicitante
/*/
user function InPZZH02()
	local aArea 	:= FwGetArea()
	local aAreaZZB 	:= ZZB->(FwGetArea())
	local cRet 		:= ""
	
	cRet := POSICIONE("ZZB",2, FWXFILIAL("ZZB")+RETCODUSR(), "ZZB_ID")
...
/*/{Protheus.doc} InPZZH03()
Retorna o Nome do Solicitante cadastrado no HelpDesk.
Preenche o campo ZZH_NOMSOL (Nome do Solicitante).
@type user function
@author user
@since 31/05/2026
@return cRet, Caractere, Nome do Solicitante
/*/
user function InPZZH03()  
	local aArea 	:= FwGetArea()
	local aAreaZZB 	:= ZZB->(FwGetArea())
	local cRet  	:= ""
	
	cRet := USRFULLNAME(POSICIONE("ZZB",1,FWxFilial("ZZB")+FWFLDGET("ZZH_IDSOL"), "ZZB_IDPROT"))
...
/*/{Protheus.doc} InPZZH04()
Retorna o ID do Departamento cadastrado no HelpDesk.
Preenche o campo ZZH_IDDEP (ID do Departamento).
@type user function
@author user
@since 31/05/2026
@return cRet, Caractere, ID do Departamento
/*/
user function InPZZH04()  
	local aArea  	:= FwGetArea()
	local aAreaZZB 	:= ZZB->(FwGetArea())
	local cRet 		:= ""
	
	cRet := POSICIONE("ZZB",1,FWXFILIAL("ZZB")+FWFLDGET("ZZH_IDSOL"),"ZZB_IDDPTO")
...
/*/{Protheus.doc} InPZZH05()
Retorna o Descricao do Departamento cadastrado no HelpDesk.
Preenche o campo ZZH_DESDEP (Descricao do Departamento).
@type user function
@author user
@since 31/05/2026
@return cRet, Caractere, Descricao do Departamento
/*/
user function InPZZH05()  
	local aArea  	:= FwGetArea()
	local aAreaZZA 	:= ZZA->(FwGetArea())
	local cRet 		:= ""
	
	cRet := POSICIONE("ZZA",1,FWXFILIAL("ZZA")+FWFLDGET("ZZH_IDDEP"),"ZZA_DESCR")
...
/*/{Protheus.doc} InPZZH06()
Retorna o Descricao do Tipo de Chamado cadastrado no HelpDesk.
Preenche o campo ZZH_DESTIP (Descricao do Tipo do Chamado).
@type user function
@author user
@since 31/05/2026
@return cRet, Caractere, Descricao do Tipo do Chamado
/*/
user function InPZZH06() 
	local aArea  	:= FwGetArea()
	local aAreaZZC 	:= ZZC->(FwGetArea())
	local cRet 		:= ""
	
	cRet := POSICIONE("ZZC",1,FWXFILIAL("ZZC")+FWFLDGET("ZZH_IDTIPO"),"ZZC_DESCR")
...
/*/{Protheus.doc} InPZZH07()
Retorna o Descricao da Categoria do Chamado cadastrado no HelpDesk.
Preenche o campo ZZH_DESCAT (Descricao da Categoria).
@type user function
@author user
@since 31/05/2026
@return cRet, Caractere, ID do Solicitante
/*/
user function InPZZH07() 
	local aArea  	:= FwGetArea()
	local aAreaZZD 	:= ZZD->(FwGetArea())
	local cRet 		:= ""
	
	cRet := POSICIONE("ZZD",2,FWXFILIAL("ZZD")+FWFLDGET("ZZH_IDCAT"),"ZZD_DESCR")
...
/*/{Protheus.doc} InPZZH08()
Retorna o Descricao da Subcategoria do Chamado cadastrado no HelpDesk.
Preenche o campo ZZH_DESSUC (Descricao da Subcategoria).
@type user function
@author user
@since 31/05/2026
@return cRet, Caractere, ID do Solicitante
/*/
user function InPZZH08()
	local aArea := FwGetArea()
	local cRet 	:= ""
	
	cRet := POSICIONE("ZZE",2,FWXFILIAL("ZZE")+FWFLDGET("ZZH_IDSUBC"),"ZZE_DESCR")
	
	FwRestArea(aArea)
return cRet


/*/{Protheus.doc} InPZZH09()
Retorna a Data corrente para preencher a data da abertura do Chamado.
Preenche o campo ZZH_DTABER (Data da Abertura do Chamado).
@type user function
@author user
@since 31/05/2026
@return cRet, Caractere, ID do Solicitante
/*/
user function InPZZH09() 
	local aArea := FwGetArea()
	local cRet 	:= ""
...
user function InPZZI01()
	local aArea 	:= FwGetArea()
	local aAreaZZI  := ZZI->(FwGetArea())
	local cIDUsrHP 	:= ""

	cIDUsrHP := POSICIONE("ZZB",2, FWXFILIAL("ZZB")+RETCODUSR(), "ZZB_ID")

	FwRestArea(aAreaZZI)
	FwRestArea(aArea)
return cIDUsrHP

/*/{Protheus.doc} InPZZI02()
Retorna o Nome do Usuario HelpDesk.
Preenche o campo ZZI_NOMUSR (Nome do Usuario).
@type user function
@author Luis Felipe Oliveira
@since 31/05/2026
@return cxNome, Caractere, Nome do usuario
/*/
user function InPZZI02()
	local cIDProt    := ""
	local cxNome     := ""

    //Pega o ID do usuario Protheus 
    cIDProt := RetCodUsr()
        
    //Pega o nome completo do usuario
    cxNome := USRFULLNAME(cIdProt)
return cxNome
